...
export const Cart = () => {
  ...
  const { products, currency, cartItems, getCartAmount, updateQuantity,navigate } = useContext(ShopContext);
    ...
    for (const items in cartItems) {
      for (const item in cartItems[items]) {
        if (cartItems[items][item] > 0) {
          data.push({ _id: items, size: item, quantity: cartItems[items][item] });
        }
      }
    }
